...
# ————————————————————
# 5. MAIN: FOR EACH FOLDER 5_seed* → FOR EACH SEED
# ————————————————————
exp_dirs = [d for d in sorted(ROOT_DIR.glob(EXPERIMENT_GLOB)) if d.is_dir()]
if not exp_dirs:
    raise SystemExit(f"No folder matched with '{EXPERIMENT_GLOB}' found it in {ROOT_DIR}")
...
for BASE_DIR in exp_dirs:
    ...
    if not ckpt_root.exists() or not res_root.exists():
        print(f"\n⏭️  {BASE_DIR.name}: missing 'checkpoints' or 'results' → skip.")
        continue
    ...
    for seed_dir in sorted(ckpt_root.glob("train_*")):
        ...
        if not ckpt_path.exists() or not all(p.exists() for p in csv_paths.values()):
            print(f"⏭️  {BASE_DIR.name}/{seed_name}: ckpt or csv missing → skip.")
            continue
