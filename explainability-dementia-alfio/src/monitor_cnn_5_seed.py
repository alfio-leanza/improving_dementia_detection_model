...
def read_inferences(csv_path, annot_df=None):
    """Legge un *_inferences.csv e restituisce un DF con almeno crop_file, pred_label, true_label."""
    df = pd.read_csv(csv_path)
    if 'crop_file' not in df.columns:
        raise KeyError(f"'crop_file' mancante in {csv_path}")
    # pred_label: se assente, derivala da softmax_values/logits
    if 'pred_label' not in df.columns:
        if 'softmax_values' in df.columns:
            ...
            raise KeyError(f"'pred_label' non presente e nessuna colonna softmax/logits trovata in {csv_path}")
    # true_label: usa quella del CSV se presente, altrimenti annot_df
    if 'true_label' not in df.columns:
        if annot_df is None:
            raise KeyError(f"'true_label' non presente in {csv_path} e annot_df=None")
        ...
        if df['true_label'].isna().any():
            ...
            raise ValueError(f"{missing} true_label non trovate via annot_df per {csv_path}")
...
def compute_class_weights(labels):
    # labels ∈ {0,1}; pesi inversamente proporzionali alla frequenza
...
# opzionale: carica annotazioni se serve
...
for exp_path in experiments:
    ...
    for seed_path in seed_dirs:
        ...
        if missing:
            print(f"[WARN] Mancano CSV in {exp_name}/{seed_name}: {missing}. Salto questo seed.")
            continue

        # --- Costruisci DataFrame con target monitor ---
        ...
        # --- Training loop ---
        print(f"\n[INFO] Training monitor per {exp_name} / {seed_name}")
        best_val_acc = -1.0
        best_state   = None
        ...
        # Ripristina best e valuta su test
        ...
        print(f"[INFO] Salvato in {out_dir}")