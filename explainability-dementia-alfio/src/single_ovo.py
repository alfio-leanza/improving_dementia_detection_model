...
# ------------------------------------------------------------------ #
def bce_masked(logit, target, mask, loss_fn):
    """
    BCEWithLogitsLoss 'none', poi media solo sui campioni validi.
    """
...
def train_one_epoch(model, epoch, tb_writer, loader, device,
    ...
    for data in tqdm(loader, ncols=100, desc='  Train'):
        ...
        _, heads = model(data.x, data.edge_index, data.batch,
                         return_all=True)            ### OVO MOD ###
        log_hc_ftd, log_hc_ad, log_ftd_ad = heads    ### OVO MOD ###

        y = data.y
        # -------- target & mask per testa -----------
        mask0 = (y != 2).float();  tgt0 = (y == 0).float()
        mask1 = (y != 1).float();  tgt1 = (y == 0).float()
        mask2 = (y != 0).float();  tgt2 = (y == 1).float()

        loss0 = bce_masked(log_hc_ftd, tgt0, mask0, loss_fn)
        loss1 = bce_masked(log_hc_ad,  tgt1, mask1, loss_fn)
        loss2 = bce_masked(log_ftd_ad, tgt2, mask2, loss_fn)
        ...
        with torch.no_grad():
            ...
            prob = model._aggregate_probs(p0, p1, p2)  # logits
...
def val_one_epoch(model, epoch, tb_writer, loader, device, loss_fn, testing=False):
    ...
    for data in tqdm(loader, ncols=100, desc=tqdm_desc):
        ...
        _, heads = model(data.x, data.edge_index, data.batch,
                         return_all=True)
        log_hc_ftd, log_hc_ad, log_ftd_ad = heads
        y = data.y
        ...
        p1 = torch.sigmoid(log_hc_ad)
        p2 = torch.sigmoid(log_ftd_ad)
        logits_final = model._aggregate_probs(p0, p1, p2)
...
def main():
    ...
    writer = SummaryWriter('/home/alfio/improving_dementia_detection_model/explainability-dementia-alfio/local/ovo_head/runs/train_{}'.format(session_timestamp))
    checkpoint_save_dir = f'/home/alfio/improving_dementia_detection_model/explainability-dementia-alfio/local/ovo_head/checkpoints/train_{session_timestamp}/'
    results_save_dir = '/home/alfio/improving_dementia_detection_model/explainability-dementia-alfio/local/ovo_head/results'
    ...
    model    = OneVsOneGNN(backbone, feat_dim=64).to(device)
    loss_fn  = torch.nn.BCEWithLogitsLoss(reduction='none')    ### OVO MOD ###
