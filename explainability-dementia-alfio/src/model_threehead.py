...
model_multihead.py
------------------
Backbone GNNCWT2D_Mk11_1sec + Multi-Head Collaborative GNN (K=3 di default).
• Accetta sia un oggetto Data sia tuple (x, edge_index, batch).
• forward(...)            -> logits medi (B,3)
• forward(..., return_all=True) -> (logits medi, lista[K] di logits)
"""
...
# ------------------------------------------------------------------ #
# 1) Edge-weighted GraphConv (softplus per pesi > 0)
# ------------------------------------------------------------------ #
class _EdgeWeightsGraphConvLayer(nn.Module):
    def __init__(self, n_edges, in_ch, out_ch):
        ...
        self.edge_w = nn.Parameter(torch.zeros(n_edges))        # init≈1 dopo softplus
...
# ------------------------------------------------------------------ #
# 2) Backbone: GNNCWT2D_Mk11_1sec (input 40×500  →  feature)
# ------------------------------------------------------------------ #
...
# ------------------------------------------------------------------ #
# 3) Multi-Head Collaborative GNN
# ------------------------------------------------------------------ #
class MultiHeadCollaborativeGNN(nn.Module):
    def __init__(self, backbone, feat_dim, num_heads=3, num_classes=3, temperature=2.0):
        super().__init__()
        self.backbone = backbone
        self.K = num_heads
        self.T = temperature
        self.heads = nn.ModuleList([nn.Linear(feat_dim, num_classes) for _ in range(num_heads)])
        for h in self.heads:
            nn.init.xavier_uniform_(h.weight); nn.init.zeros_(h.bias)

    # -------------------------------------------------------------- #
    def forward(self, *args, return_all=False):
        """
        Supporta:
           • forward(Data, ...)                (oggetto torch_geometric.data.Data)
        ...
        logits_list = [head(feat) for head in self.heads]                  # K × (B,3)
        avg_logits  = torch.stack(logits_list, 0).mean(0)                  # (B,3)
