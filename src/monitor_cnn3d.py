...
class CNN3D_ChannelAttention(nn.Module):
    def __init__(self, in_channels=1, depth=19, num_classes=2):
        ...
        self.conv_block = nn.Sequential(
            nn.Conv3d(in_channels, 16, kernel_size=(3,3,3), padding=(1,1,1), bias=False),
            nn.BatchNorm3d(16),
            nn.ReLU(),
            nn.MaxPool3d((1,2,2)),
            nn.Dropout3d(0.2),
            ...
            nn.BatchNorm3d(64),
            nn.ReLU(),
            nn.AdaptiveAvgPool3d((depth,1,1))
        )
        self.channel_attention = nn.Sequential(
            nn.Conv3d(64, 16, kernel_size=(depth,1,1), bias=False),
            nn.ReLU(),
    ...
    def forward(self, x):
        feat = self.conv_block(x)            # [B,64,19,1,1]
        attn = self.channel_attention(feat)  # [B,64,1,1,1]
        weighted = feat * attn
...
class CWT3D_Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        data = np.load(os.path.join(self.root_dir, fname))
        if self.augment:
...
def save_history_plots(history, output_dir):
    ...
    plt.savefig(os.path.join(output_dir,'training_history_loss.png')); plt.close()
    ...
    plt.savefig(os.path.join(output_dir,'training_history_acc.png')); plt.close()
...
def test_and_save_predictions(model, loader, device, output_dir):
    ...
    plt.title('Confusion Matrix'); plt.ylabel('True'); plt.xlabel('Pred'); plt.savefig(os.path.join(output_dir,'confusion_matrix.png'))
    plt.close(); print(f"Test Loss={test_loss:.4f}, Test Acc={test_acc:.4f}")
...
def main():
    ...
    def create_loader(split, augment=False):
        ...
        return DataLoader(CWT3D_Dataset(list(subset['crop_file']), list(subset['train_label']), cwt_root, augment), batch_size=16, shuffle=True)
